...
The server publishes data for the client to use, and the client subscribes to the server's published data.  It's difficult to understand the relationship between the server's published data and the client's subscription at first.  The rule of thumb is this: **The client should only be able to access data that it requires to operate at the current point in time, nothing more**.  For example, if you have a chatroom application, the client should *not* be receiving messages from every chat channel on your website, but only the messages in the channel that the user is visiting. Nor should it know about users inside other channels.
