---
layout: post
title: Learn Meteor Fundamentals
category:
...
## Introduction
Want to learn how this new, fandangled [Meteor](http://meteor.com/) thing works? Great, you're in the right spot. I want to show you around a Meteor project and give you some best practice tips for you to keep in mind when you want to make your own.
...
For storing data, Meteor uses something that's pretty close to [MongoDB](http://www.mongodb.org/).  Meteor actually uses minimongo, which is fairly full featured but doesn't exactly have all of the good stuff from regular MongoDB.  You don't need to actually know how MongoDB works exactly, but I would recommend taking a look at Meteor's [docs for Collections](http://docs.meteor.com/#collections) to get a good idea of what you're in for.
...
There are times that you want to separate your server code from your client code.  Thankfully, Meteor has a couple flags to help you out: `Meteor.is_server` and `Meteor.is_client`.
...
Templates are reactive contexts: if the data that it uses to render itself has a reactive data source, then it will re-render itself whenever that data source changes.  The client-side `Session` is a reactive data source.  It will store information like a key-value pair on the client only, and it will be erased when the page is refreshed.
...
***Note:*** *In your project directory, execute* `$ meteor remove autopublish` *on every project. It publishes all your data by default to the client and is poor practice.*

The server publishes data for the client to use, and the client subscribes to the server's published data.  It's difficult to understand the relationship between the server's published data and the client's subscription at first.  The rule of thumb is this: **The client should only be able to access data that it requires to operate at the current point in time, nothing more**.  For example, if you have a chatchannel, the client should *not* be receiving messages from every chatchannel on your website, but only the messages in the chatchannel that the user is visiting. Nor should it know about users outside of this chatchannel.
...
Now, when the client connects, when it does a `Messages.find({})` to get a list of messages, and it only gets the ones listed in the `"cool_people_channel"` channel.
